...
Данный проект является частью выполнения РГЗ по дисциплине "Проектирование систем реального времени".

[Ссылка на клиентскую часть проекта](https://github.com/AlexsandraMK/rgz_psrv/tree/sasha/front)
...
### ❗ВНИМАНИЕ
> В конце любого запроса, после контрольной суммы, должен быть символ переноса строки `\n`. 
Сервер прослушивает порт до тех пор, пока не встретит этот символ.
...
### ❗ВНИМАНИЕ
> В конце каждого ответа, после контрольной суммы, стоит символ переноса строки `\n`. 
Учитывайте этот факт, при чтении буфера порта.

### Список кодов ответов ###

| Код ответа |         Служебное сообщение          | Описание | 
|:-----------|:------------------------------------:|  :--- |
| 000        |            *отсутствует*             |Ответ содержит файл|
...
> |-|-|-|-|
> |`ЛОГИН`|Обязательный|Строка (макс. 20 символов)|Логин клиента|
> |`ПАРОЛЬ`|Обязательный|Строка (макс. 10 символов)|Пароль клиента|
...
> |Название|Тип|Тип данных|Описание|
> |-|-|-|-|
> |`ПУТЬ`|Необязательный|Строка до 1018 символов|Путь к необходимому каталогу, который клиент хочет просмотреть. Должен быть относительным к текущему каталогу клиента|
...
<details>
<summary><code>cd</code> - переход в заданный каталог</summary>


##### Аргументы
> |Название|Тип|Тип данных|Описание| 
> |-|-|-|-| 
> |`ПУТЬ`|Обязательный|Строка до 1015 символов|Путь к каталогу, в которой клиент хочет перейти. Путь должен быть относительным к текущему каталогу клиента.|
...
> </table>
>
> ❗️Важно помнить, что перед аргументом должен быть пробел.
...
>            <td><code>КС</code></td>
>            <td>12 байт</td>
>            <td>Команда выполнена успешно.  Осуществлен переход по заданному пути</td>
>        </tr>
>        <tr>
>            <td><code>102</code></td>
>            <td><code>ㅤ</code></td>
...
>            <td><code>КС</code></td>
>            <td>36 байт</td>
>            <td>В запросе количество аргументов не равно 1</td>
>        </tr>
>        <tr>
>            <td><code>149</code></td>
>            <td><code>ㅤ</code></td>
...
<details>
<summary><code>get</code> - скачивание файла с сервера</summary>


##### Аргументы
> |Название|Тип|Тип данных|Описание|
> |-|-|-|-|
> |`ИМЯ_ФАЙЛА`|Обязательный|Строка до 1014 символов|Название файла (включая расширение) в текущем каталоге клиент|
...
>            <td><code>OK</code></td>
>            <td><code>.ㅤ</code></td>
>            <td><code>File follows - КОЛИЧЕСТВО_БАЙТ bytes</code></td>
>            <td><code>.ㅤ</code></td>
>            <td><code>КС</code></td>
>            <td>до 1024 байт</td>
>            <td>Команда выполнена успешно. Следует второй ответ</td>
...
>            <td><code>103</code></td>
>            <td><code>ㅤ</code></td>
>            <td><code>ERR Unsupported file</code></td>
>            <td><code>.ㅤ</code></td>
>            <td><i>нет</i></td>
>            <td><i>нет</i></td>
>            <td><code>КС</code></td>
>            <td>30 байт</td>
>            <td>Запрашиваемый файл имеет не поддерживаемое расширение.</td>
>        </tr>
>        <tr>
>            <td><code>104</code></td>
>            <td><code>ㅤ</code></td>
>            <td><code>ERR File could not be sent</code></td>
>            <td><code>.ㅤ</code></td>
>            <td><i>нет</i></td>
>            <td><i>нет</i></td>
>            <td><code>КС</code></td>
>            <td>36 байт</td>
>            <td>Возникла ошибка при открытии или кодировании файла.</td>
>        </tr>
>        <tr>
>            <td><code>149</code></td>
>            <td><code>ㅤ</code></td>
...
>            <td><code>КС</code></td>
>            <td>46 байт</td>
>            <td>В аргументе <code>ИМЯ_ФАЙЛА</code> записано название несуществующего файла в текущем каталоге клиента.</td>
>        </tr>
>    </tbody>
> </table>
> 
...
>            <td><code>000</code></td>
>            <td><code>ㅤ</code></td>
>            <td><code>ФАЙЛ</code></td>
>            <td><code>.ㅤ</code></td>
>            <td><code>КС</code></td>
>            <td><code>КОЛИЧЕСТВО_БАЙТ + 10</code> байт</td>
>            <td>Сообщение содержит закодированный файл</td>
>        </tr>
>    </tbody>
> </table>
>
...
<details>
<summary><code>quit</code> - завершении сессии клиента</summary>

##### Аргументы
> *отсутствуют*. 
> 
>Если в запросе будут введены аргументы, они никак не будут учтены при обработке запроса.
...
При выполнении команды сессия клиента завершается. Информация о текущем каталоге клиента завершается. Чтобы клиент снова мог просматривать и скачивать содержимое с сервера, ему надо повторно авторизоваться.
