...
Если клиент отправляет несуществующую команду, сервер отправляет ответ с кодом `102`.


### hello

**Назначение:** проверка соединения с сервером.

**Аргументы:** отсутствуют.

**Структура запроса:** `hello. КОНТРОЛЬНАЯ_СУММА` (15 байт).

**Структура ответа:** `200 OK. КОНТРОЛЬНАЯ_СУММА` (12 байт).

### auth

**Назначение:** авторизация клиента.

**Аргументы:** логин, пароль.

**Структура запроса:** `auth ЛОГИН ПАРОЛЬ. КОНТРОЛЬНАЯ_СУММА`.

**Структура ответа:** `КОД СЛУЖЕБНОЕ_СООБЩЕНИЕ. КОНТРОЛЬНАЯ_СУММА`.

**Возвращаемые коды:**

* `101`, если произошла ошибка авторизации (неверные логин или пароль, отсутствие учётной записи).
* `102`, если в запросе не 2 аргумента.
* `200`, если авторизация прошла успешно.

**Пример запроса:**
```commandline
auth user1 myPassword. B40A
```
**Пример ответа:**
```
200 ОК. 004A
...
**Назначение:** возвращает путь к текущему каталогу клиента.

**Аргументы:** отсутствуют. Если в запросе будут введены аргументы, они никак не будут учтены при обработке запроса.

**Структура запроса:** `pwd. КОНТРОЛЬНАЯ_СУММА`.

**Структура ответа:** `200 ОК. ТЕКУЩИЙ_КАТАЛОГ. КОНТРОЛЬНАЯ_СУММА`.
...
**Назначение:** просмотр содержимого каталога.

**Аргументы:** либо отсутствует, либо путь к необходимому каталогу, который должен быть относительным к текущему каталогу клиента.

**Структура запроса:** 
* `ls. КОНТРОЛЬНАЯ_СУММА`.
* `ls ПУТЬ. КОНТРОЛЬНАЯ_СУММА`.
...
**Структура первого ответа:** `200 OK. КОЛИЧЕСТВО_БАЙТ bytes. КОНТРОЛЬНАЯ_СУММА`.

`КОЛИЧЕСТВО БАЙТ` это число - количество байт, которая занимает строка `СОДЕРЖИМОЕ_КАТАЛОГА` в следующем ответе.

**Структура второго ответа:** `000 СОДЕРЖИМОЕ_КАТАЛОГА. КОНТРОЛЬНАЯ_СУММА`.

**Возвращаемые коды:**

* `102`, если в запросе более одного аргумента.
...
**Пример запроса без аргумента:**
```commandline
ls. A50A
```

**Пример запроса с аргументом:**
```commandline
ls myPhotos. B004
```
